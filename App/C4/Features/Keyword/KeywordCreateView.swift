            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                HStack(alignment: .top, spacing: 24) {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        HStack(spacing: 8) {
                            CustomTextField(placeholder: "YYYY.MM.DD", text: $viewModel.draftStartDate)
                            Text("—")
                                .foregroundColor(.gray)
                            CustomTextField(placeholder: "YYYY.MM.DD", text: $viewModel.draftEndDate)
                        }
                    }
                    .frame(maxWidth: .infinity)
                }
                ...
                .fileImporter(
                    ...
                    case .success(let urls):
                        ...
                        let newAttachment = Attachment(fileName: fileName, storedFileName: UUID().uuidString + "." + fileType, fileType: fileType, fileSize: fileSize)
                        viewModel.draftAttachedFiles.append(newAttachment)
                ...
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    TextField("막막하다면 아래 질문에 대답하듯 의식의 흐름대로 적어보세요!\n\nQ. 이 프로젝트에서 나의 메인 역할은 무엇이었나요?\nQ. 진행 중 겪었던 가장 빡쳤던(?) 위기나 한계는 무엇이었나요?\nQ. 그 위기를 넘기기 위해 '나'는 구체적으로 어떤 고민과 행동을 했나요?\nQ. 결과적으로 무엇을 이뤄냈고, 무엇을 배웠나요?", text: $viewModel.draftStatement, axis: .vertical)
                        .textFieldStyle(.plain)
                        .lineLimit(8...15)
                        .font(Font.custom("SF Pro", size: 13))
                        .padding(16)
